<template>
  <div class="page">
    ...
    <mt-popup v-model="popupVisible4" position="bottom" class="mint-popup">
      ...
      <select-data :data="floorList" v-if="popupVisible4" @chooseInfo="chooseInfo"></select-data>
    </mt-popup>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    //     floorId: this.id
    //   }
    //   window.$findFloorTriangle(params).then((res) => {
    //     this.data = res
    //     this.dataRecombined()
    //   }, (err) => {console.log(err)})
    // },
    ...
    getDetails() {
      ...
      console.log("当前楼层的id："+this.currentFloorID)
    ...
    setTitle () {
      ...
      if(this.currentFloorID !== this.temporaryData.id){
        this.currentFloorID = this.temporaryData.id
        this.getDetails()
      }
